...
async function generateDomain(config: DomainConfig): Promise<void> {
  ...
  if (sharedTypes.length > 0) {
    ...
    if (sharedTypeImports.length > 0) {
      sharedFile = `import type { ${sharedTypeImports.join(`, `)} } from '../types';\n\n${sharedFile}`;
    }
  ...
  // Generate cypress intercept helpers if configured
  if (config.cypressInterceptFile) {
    ...
    console.log(`  Generated cypress intercept: ${config.cypressInterceptFile}`);
  }
}
...
function generateCypressInterceptFile(
  ...
  // Implementation
  lines.push(`export function interceptPql(slug: string, output: any): void {`);
  lines.push(
